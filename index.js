...
const requireGulpfile = () => {
    ...
    const gulpFilename = fs
        .readdirSync(projectRoot)
        .find(f => /^gulpfile.js$/i.test(f));
...
const isFirstInvocation = () =>
    invokingFilePath() === primaryInvokingFilePath();
...
function register(registrationFunc) {
    ...
    if (!isJustMainTask(taskName, task.tasks)) {
        // We chain the Futures so that tasks can be executed sequentially
        // rather than in parallel. Many of these tasks deal with
        // the file system and it is not desirable that they
